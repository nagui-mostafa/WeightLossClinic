...
describe('ObjectStorageService', () => {
  ...
  const createService = (
    ...
    const config: Configuration['objectStorage'] = { ...baseConfig, ...overrides };
  ...
  it('builds S3-style URLs when driver is s3', () => {
    const service = createService({ driver: 's3', bucket: 'cdn-bucket', region: 'us-west-2' });
    const url = service.getPublicUrl('assets/hero.png');
    expect(url).toBe('https://cdn-bucket.s3.us-west-2.amazonaws.com/assets/hero.png');
  });

  it('builds MinIO path-style URLs when configured', () => {
    const service = createService({ endpoint: 'http://127.0.0.1:9000', forcePathStyle: true });
  ...
  it('normalizes object keys', () => {
    ...
    expect(service.normalizeObjectKey('  /folder/file.png ')).toBe('folder/file.png');
